...
import style from './separateFamily.module.css'
      ...
      <div className={style.heading} style={{ marginBottom: "10px"}}>Separate Family</div>
        ...
        {getFamilyListData?.content?.length > 0 ?  <div className={style.tablewrapper} >
          <table className={style.table}>
            ...
            <tbody>{getFamilyListData?.content?.filter(f => f?.totalMembers > 0)?.map(v => (
              <tr className={style.tr}>
                ...
                <td className={style.td}><div className={style.btns}>
                  <SubmitButton label={"Separate"} onClick={() => handleClickOpen(v)} />
                    {/* <p style={{cursor : "pointer", color : "blue"}} onClick={() => handleClickOpen(v)}>Separate</p> */}
...
// export default SeparateFamily
export default withAuth(SeparateFamily)
